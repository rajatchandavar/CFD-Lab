...
template <typename T> class Matrix {
  ...
  public:
     ...
     *
     */
    Matrix<T>(int i_max, int j_max, dtype init_val) : _imax(i_max), _jmax(j_max) {
    ...
    std::vector<dtype> get_row(int row) {
    ...
    std::vector<dtype> get_col(int col) {
    ...
    void set_col(const std::vector<dtype> &vec, int col) {
        for (int i = 0; i < _jmax; ++i) {
    ...
    void set_row(const std::vector<dtype> &vec, int row) {
        for (int i = 0; i < _imax; ++i) {
